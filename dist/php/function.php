<?php
  ...
  function join_users($user_name, $user_id, $user_pwd, $user_email, $user_gender, $user_birth, $user_addr) {
    ...
    VALUE ('{$user_name}', '{$user_id}', '{$user_pwd}', '{$user_email}', '{$user_gender}', '{$user_birth}', '{$user_addr}', {$modify_time}, '{$create_time}')" ;
  ...
  //查詢註冊姓名是否已使用
  function checked_user_name($user_name) {
    $result = null ;
  ...
  //查詢註冊id是否已使用
  function checked_user_id($user_id) {
    $result = null ;
